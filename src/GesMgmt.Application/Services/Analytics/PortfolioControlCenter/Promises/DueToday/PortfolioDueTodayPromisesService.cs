...
public sealed class PortfolioDueTodayPromisesService(
    IPortfolioControlCenterAccessService accessService,
    IPortfolioPromisesRepository promisesRepository,
    IPortfolioDueTodayPromisesRepository repository) : IPortfolioDueTodayPromisesService
{
    public async Task<PortfolioOperationResult<PortfolioDueTodayPromisesResponse>> GetAsync(
        string? campaign,
    ...
    {
        if (!PortfolioDueTodayPromisesRequest.TryCreate(
                campaign,
                subPortfolioId,
                page,
                pageSize,
        ...
        {
            return PortfolioOperationResult<PortfolioDueTodayPromisesResponse>.Validation(requestErrors);
        }
        ...
        {
            return PortfolioOperationResult<PortfolioDueTodayPromisesResponse>.Validation(businessUnitErrors);
        }
        ...
        {
            return PortfolioOperationResult<PortfolioDueTodayPromisesResponse>.FromAccess(clientAccess);
        }
        ...
        if (context is null)
        {
            return PortfolioOperationResult<PortfolioDueTodayPromisesResponse>.Problem(
                statusCode: 404,
        ...
        return PortfolioOperationResult<PortfolioDueTodayPromisesResponse>.Success(
            PortfolioDueTodayPromisesResponseMapper.Map(
                context,
                result));
