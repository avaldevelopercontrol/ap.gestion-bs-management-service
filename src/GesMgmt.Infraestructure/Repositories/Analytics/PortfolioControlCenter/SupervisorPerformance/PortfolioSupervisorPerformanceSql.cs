...
internal static class PortfolioSupervisorPerformanceSql
{
    public const string SupervisorPerformance = """
        ...
        (
            SELECT
                ...
                COUNT(DISTINCT advisor_key) AS advisor_count,
        ...
        (
            ...
            WHERE c.client_key = @ClientKey
              AND c.supervisor_key IS NOT NULL
        ...
        (
            ...
            WHERE p.client_key = @ClientKey
              AND p.supervisor_key IS NOT NULL
        ...
        (
            SELECT
                ...
                COALESCE(SUM(fulfillment_paid_amount), 0) AS fulfillment_paid_amount,
                COALESCE(SUM(fulfillment_promise_amount), 0) AS fulfillment_promise_amount,
                MAX(loaded_at) AS loaded_at
        ...
        SELECT
            m.supervisor_key AS SupervisorId,
            m.supervisor_name AS SupervisorName,
            er.date_from AS DateFrom,
            er.date_to AS DateTo,
            CONVERT(BIGINT, ISNULL(m.advisor_count, 0)) AS AdvisorCount,
            ...
            CAST(
                ISNULL(pm.fulfillment_paid_amount, 0)
        ...
        LEFT JOIN ContactMetrics AS c
            ON c.supervisor_key = m.supervisor_key
        LEFT JOIN PromiseMetrics AS pm
            ON pm.supervisor_key = m.supervisor_key
        LEFT JOIN PaymentMetrics AS pay
            ON pay.supervisor_key = m.supervisor_key
        CROSS APPLY
        (
        ...
        ORDER BY m.supervisor_name, m.supervisor_key;
        """;
}
